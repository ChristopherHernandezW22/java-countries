...
public class CountryList {
    ...
    // methods that use the checkcountry interface

    // method that returns a list

    public ArrayList<Country> getCountryList (CheckCountry tester){
        // make a temporary array list to hold only the countries we want returned
        ArrayList<Country> tempCountryList = new ArrayList<>();

        // now loop through all the countries and add to the temp list and return it
    ...
    // method that return only one country

    public Country findCountry(CheckCountry tester){
        // go through list and return the first option that is true other return null
        for(Country c : countryList){
            // if true
            if(tester.test(c)){
                return c;
